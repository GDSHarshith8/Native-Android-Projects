...
import com.taskmaker.presentation.viewModels.DeleteTaskVM
...
fun DelTS(
    ...
    val deleteVM = hiltViewModel<DeleteTaskVM>()
    val allTasksVM = hiltViewModel<AllTasksVM>()
        ...
        LazyColumn(
            ...
            items(tasks.size) { index ->
                ...
                DeleteTaskCard(task) {
                    ...
                    // ✅ 1. Remove from UI and DB
                    tasks.remove(task)
                    deleteVM.deleteTask(task)

                    // ✅ 2. Launch undo snackbar
                    scope.launch {
                        showUndoDeleteSnackbar(sbh, task) {
                            // ✅ 3. Undo: Add back to UI and DB
                            tasks.add(task)
                            allTasksVM.insertTask(task)
